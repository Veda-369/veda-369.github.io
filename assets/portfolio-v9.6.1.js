...
// Storage is intentionally wrapped because privacy settings/local file previews can deny sessionStorage.
const safeSessionGet=key=>{try{return sessionStorage.getItem(key)}catch(_e){return null}};
const safeSessionSet=(key,value)=>{try{sessionStorage.setItem(key,value)}catch(_e){/* storage unavailable: site must still work */}};
const revealSite=entryEl=>{
  if(entryEl&&entryEl.isConnected)entryEl.remove();
...
if(entry&&shutter){
  ...
  const sessionEntered=safeSessionGet('veda_entered')==='1';
  const skipIntro=sessionEntered||Boolean(location.hash)||params.get('from')==='photo';
  if(skipIntro){
    safeSessionSet('veda_entered','1');
    revealSite(entry);
    if(params.get('from')==='photo'){try{history.replaceState({},'',location.pathname+(location.hash||''))}catch(_e){}}
  }else{
    document.body.classList.add('entry-locked');
    shutter.addEventListener('click',()=>{
      if(entry.classList.contains('capturing'))return;
      try{playShutter()}catch(_e){}
      entry.classList.add('capturing');shutter.disabled=true;
      safeSessionSet('veda_entered','1');
      setTimeout(()=>{if(entry.isConnected)entry.classList.add('exit')},690);
      setTimeout(()=>revealSite(entry),1150);
    },{once:true});
  }
}
...
// Homepage photography highlights: tolerate .jpg / .jpeg / .png / .webp naming.
function homePhotoCandidates(n){
  const bases=[`photos/photo ${n}`,`photos/photo${n}`];
  const exts=['.jpg','.jpeg','.png','.webp','.JPG','.JPEG','.PNG','.WEBP'];
  return bases.flatMap(base=>exts.map(ext=>base+ext));
}
qa('img[data-home-photo]').forEach(img=>{
  const n=Number(img.dataset.homePhoto);if(!Number.isFinite(n))return;
  const current=img.getAttribute('src');
  const candidates=[current,...homePhotoCandidates(n).filter(src=>src!==current)];
  let at=1;
...
function createGalleryVideo(){
  const fig=document.createElement('figure');
  fig.className='gallery-video-frame reveal';
  const video=document.createElement('video');
  video.autoplay=true;video.muted=true;video.loop=true;video.playsInline=true;
  video.preload='metadata';video.poster='videos/video1-poster.jpg';
  video.setAttribute('disablepictureinpicture','');
  video.setAttribute('controlslist','nodownload noplaybackrate nofullscreen');
  video.innerHTML='<source src="videos/video1.mp4" type="video/mp4">';
...
if(gallery){
  (async()=>{
    ...
    files.forEach((file,i)=>{
      gallery.appendChild(createPhotoFigure(file,i));
      if(i===4)gallery.appendChild(createGalleryVideo());
    });
    if('IntersectionObserver'in window){
      ...
      qa('.photo-item,.gallery-video-frame').forEach(el=>photoIO.observe(el));
    }else qa('.photo-item,.gallery-video-frame').forEach(el=>el.classList.add('visible'));
  })();
}
...
document.addEventListener('contextmenu',e=>{if(e.target.closest('.photo-grid img,.photo-feature img,.lightbox img,.gallery-video-frame video,.home-video video'))e.preventDefault()});
document.addEventListener('dragstart',e=>{if(e.target.closest('.photo-grid img,.photo-feature img,.lightbox img,.gallery-video-frame video,.home-video video'))e.preventDefault()});
