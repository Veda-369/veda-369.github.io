...
qa('a[href]').forEach(a=>a.addEventListener('click',e=>{
  ...
  if(!href||href.startsWith('#')||href.startsWith('mailto:')||href.startsWith('http')||a.target==='_blank'||href.endsWith('.pdf')) return;
...
const beam=q('.hero-beam'); if(beam&&matchMedia('(pointer:fine)').matches) addEventListener('mousemove',e=>{const dx=(e.clientX/innerWidth-.5)*22,dy=(e.clientY/innerHeight-.5)*22;beam.style.transform=`translate(${dx}px,${dy}px)`});

// Photography lightbox
const lb=q('.lightbox'); if(lb){
  ...
  shots.forEach((s,i)=>s.addEventListener('click',()=>show(i))); close.addEventListener('click',()=>{lb.classList.remove('open');document.body.style.overflow=''}); prev.addEventListener('click',()=>show(idx-1));next.addEventListener('click',()=>show(idx+1));
...
// contact form status (keeps FormSubmit workflow)
